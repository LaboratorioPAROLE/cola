...
server <- function(input, output, session) {
  ...
  observeEvent(input$open_compare, {
    showModal(modalDialog(
      ...
      withSpinner(
        ...
        type = 6,           # Stile dell'animazione
        color = "#0d6efd"   # Colore blu di default
      ),
  ...
  iaa_data <- reactive({
    ...
    # Barra di avanzamento pop-up con percentuali
    withProgress(message = "Calcolo IAA in corso...", value = 0, {
      
      # FASE 1: Caricamento e allineamento (20%)
      ...
      # FASE 3: Calcolo Macro e Microfunzioni (80%)
      ...
      if (n_fn > 0) {
        ...
        res_macro$positive <- calc_macro_positive(m1_macro, m2_macro)
        
        res_micro <- calc_multilabel_matching(m1, m2, ALL_KEYS)
        res_micro$positive <- calc_micro_positive(m1, m2)
        
        per_label <- per_label_df(m1, m2, ALL_KEYS)
        res_poly <- calc_polyfunctionality(m1, m2)
        
        macro_detail <- per_label_df(
          m1_macro, m2_macro, c("Interazionali", "Metatestuali", "Cognitive")
        )
      }
      
      # FASE 4: Finalizzazione (100%)
  ...
  output$iaa_results_ui <- renderUI({
    ...
    tt_positive <- "Accordo calcolato escludendo i casi in cui entrambi gli annotatori hanno assegnato 0."
      ...
      tags$table(
        ...
        tags$thead(
          tags$tr(
            ...
            tags$th(lbl_info("Cohen Kappa", "Non applicato direttamente alle rappresentazioni set-valued.")),
            tags$th(lbl_info("Gwet AC1", "Non applicato direttamente alle rappresentazioni set-valued.")),
    ...
    make_positive_table <- function(res) {
      ...
      tags$table(
        ...
        tags$thead(
          tags$tr(
            tags$th(lbl_info("Misura", tt_positive)),
            tags$th("Valore")
          )
        ),
        tags$tbody(
          tags$tr(tags$td("Accordo positivo-condizionato"), tags$td(ifelse(is.na(res$po), "—", sprintf("%.2f%%", res$po)))),
          tags$tr(tags$td("Cohen's Kappa+"), tags$td(ifelse(is.na(res$kappa), "—", round(res$kappa, 3)))),
          tags$tr(tags$td("Gwet's AC1+"), tags$td(ifelse(is.na(res$ac1), "—", round(res$ac1, 3)))),
          tags$tr(tags$td("Krippendorff's Alpha+"), tags$td(ifelse(is.na(res$alpha), "—", round(res$alpha, 3))))
        )
      )
    }
    ...
    tagList(
      ...
      tabsetPanel(
        ...
        tabPanel(
          ...
          h5("Accordo positivo-condizionato sulle macrofunzioni"),
          make_positive_table(d$res_macro$positive), br(),
        ...
        tabPanel(
          ...
          h5("Accordo positivo-condizionato — aggregazione Micro"),
          make_positive_table(d$res_micro$positive), br(),
